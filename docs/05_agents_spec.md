...
1. **SCM Agent 수직슬라이스**(L1→L3): 판단(mock)→봉투→브리지→08/ERP어댑터(mock)→감사→업무기억. ← 대표 시나리오(docs/00 §3).
2. 공통 골격을 템플릿화 → 나머지 5 Agent 수평 복제.
3. Agent Memory 4종 인덱스: live Databricks 확보 시. mock은 스텁 회수.
